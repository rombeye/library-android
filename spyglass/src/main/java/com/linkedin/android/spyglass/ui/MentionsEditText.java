...
public class MentionsEditText extends EditText implements TokenSource {
    ...
    private boolean hashTagsEnabled = false;

    private int mPreviousLength;
    boolean wroteHashTag = false;
    ...
    public void enableHashTags(boolean hashTagsEnabled) {
    ...
    private class MyWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable text) {
            //check if the user change the text or programmatically
            if (isTextChangedByUser) {
            ...
            cursorPosition = getSelectionStart();
            //check if any char has deleted
            if (hashTagsEnabled && previousText.length() > text.length() && text.length() > 0) {
                //Compare the old chars with new ones
                int i = 0;
                do {
                    if (previousText.charAt(i) != text.charAt(i) && previousText.charAt(i) == '#') {
                        //this is removed hashtag
                        int j = i + 1;
                        while ((j < text.length() && text.charAt(j) != ' ' && j != text.length()) || j == text.length())
                            j++;

                        Spannable spannable = getText();
                        ForegroundColorSpan[] toRemoveSpans = spannable.getSpans(i, j, ForegroundColorSpan.class);
                        for (int index = 0; index < toRemoveSpans.length; index++)
                            spannable.removeSpan(toRemoveSpans[index]);
                        StyleSpan[] toRemoveSpans1 = spannable.getSpans(i, j, StyleSpan.class);
                        for (int index = 0; index < toRemoveSpans1.length; index++)
                            spannable.removeSpan(toRemoveSpans1[index]);
            ...
            // add hashtags
            if (hashTagsEnabled) {
                //String newText = ArabicUtilities.reshape(getText().toString());
                //Spannable spannable = new SpannableString(newText);
                Spannable spannable = getText();
                for (Hashtag hashtag : getHashTags(getMentionsText())) {
                    //CharSequence hashtagText = text.subSequence(hashtag.getStartsAt(), hashtag.getEndsAt());
                    //text.replace(hashtag.getStartsAt(),hashtag.getEndsAt(),hashtag.getText());
                    if (hashtag.getStartsAt().intValue() != hashtag.getEndsAt().intValue()) { // this condition is to prevent coloring of one hastag only
                        spannable.setSpan(new ForegroundColorSpan(ContextCompat.getColor(getContext(), R.color.turquoise)), hashtag.getStartsAt(), hashtag.getEndsAt(), Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                        spannable.setSpan(new StyleSpan(Typeface.NORMAL), hashtag.getStartsAt(), hashtag.getEndsAt(), Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                    }
                }
                if (isGoogleKeyboard()) {
                    for (Hashtag hashtag : getHashTags(getMentionsText())) {
    ...
    public boolean isGoogleKeyboard() {
        String googlePackageName = "com.google.android.inputmethod.latin";
        final String defaultIME = Settings.Secure.getString(getContext().getContentResolver(), Settings.Secure.DEFAULT_INPUT_METHOD);
        ComponentName defaultInputMethod = ComponentName.unflattenFromString(defaultIME);
        if (defaultInputMethod.getPackageName().equals(googlePackageName)) {
            return true;
        } else {
            return false;
        }
    ...
    public static ArrayList<Hashtag> getHashTags(MentionsEditable editable) {
        ArrayList<Hashtag> hashtags = new ArrayList<>();
        int textLength = editable.length();
        boolean hashtagStarted = false;
        int startIndex = -1;
        for (int i = 0; i < textLength; i++) {
            if (editable.charAt(i) == '#') {
                if (!hashtagStarted) {
                    ...
                    if (i - startIndex == 1) {
                        ...
                        //save current hashtag and start new one
                        Hashtag hashtag = new Hashtag();
                        char[] hashTagText = new char[i - startIndex];
                        editable.getChars(startIndex, i, hashTagText, 0);
                        hashtag.setText(hashTagText.toString());
                        hashtag.setStartsAt(startIndex);
                        hashtag.setEndsAt(i - 1);
                        if (hashtag.getText().trim().length() > 1) // prevents a single hashtag symbol from being added
            ...
            } else if (hashtagStarted && ((!Character.isLetterOrDigit(editable.charAt(i)) && editable.charAt(i) != '_') || i == editable.length() - 1)) {
                if (i == editable.length() - 1)
                    i++;
                Hashtag hashtag = new Hashtag();
                char[] hashTagText = new char[i - startIndex];
                editable.getChars(startIndex, i, hashTagText, 0);
                hashtag.setText(String.valueOf(hashTagText));
                hashtag.setStartsAt(startIndex);
                hashtag.setEndsAt(i);
    ...
    public void updateSpan(MentionSpan span) {
        ...
        mBlockCompletion = false;
        Log.i("AlaaAaa", "Span Start: " + start + " Span End: " + end);
    }
    ...
    public void insertMentionInternal(@NonNull Mentionable mention, @NonNull Editable text, int start, int end) {
...
//        text.setSpan(new ForegroundColorSpan(ContextCompat.getColor(getContext(), R.color.black)), start, endOfMention, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        text.setSpan(new StyleSpan(Typeface.BOLD), start, endOfMention, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
