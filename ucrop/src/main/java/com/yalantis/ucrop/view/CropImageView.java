...
import com.yalantis.ucrop.MessageEvent;
...
public class CropImageView extends TransformImageView {
    ...
    boolean isExpanded = false, isMultiple = false, isOldFragment = false;
    Matrix oldMatrix = new Matrix();
    private final RectF firstSelectedRectCrop = new RectF();

    public ChangeRatioCallback changeRatioCallback;
...
//        if (isMultiple) {
//            cropRect.set(firstSelectedRectCrop.left, firstSelectedRectCrop.top, firstSelectedRectCrop.right, firstSelectedRectCrop.bottom);
//        }
        ...
        Log.d("###setCropRect re:", mCropRect.toString());
        Log.d("###setCropRect asp:", mTargetAspectRatio + "");
    ...
    public void postScale(float deltaScale, float px, float py) {
        ...
        if (deltaScale == 1.00 && !isExpanded && !isMultiple) {
    ...
    public void setImageToWrapCropBounds(boolean animate) {
        Log.d("&&&", "mBitmapLaidOut" + mBitmapLaidOut + "isImageWrapCropBounds" + isImageWrapCropBounds());

        if (mBitmapLaidOut && !isImageWrapCropBounds()) {
            Log.d("&&&", "ismultiple" + isMultiple);
//            if (isMultiple) {
//                mCropRect.set(firstSelectedRectCrop);
//            }
    ...
    public void onImageLaidOut() {
        ...
        mTargetAspectRatio = (float) drawableWidth / drawableHeight;

        //edge case
        if (mTargetAspectRatio > 1.923f)
            mTargetAspectRatio = 1.923f;
        if (mTargetAspectRatio < 0.8f)
            mTargetAspectRatio = 0.8f;

        if (isExpanded)
            mTargetAspectRatio = 1.0f;
        ...
        if (isMultiple) {
            mCropRect.set(firstSelectedRectCrop.left, firstSelectedRectCrop.top, firstSelectedRectCrop.right, firstSelectedRectCrop.bottom);
        }
    ...
    private void calculateImageScaleBounds(float drawableWidth, float drawableHeight) {
        ...
        float originalMinScale = Math.min(widthScale, heightScale);

//        mMinScale = isExpanded ? fitZoomScale : originalMinScale;
        mMinScale = originalMinScale;

        mMaxScale = mMinScale * mMaxScaleMultiplier;
//        zoomImageToPosition(mMinScale, mCropRect.centerX(), mCropRect.centerY(), 200);
    }
    ...
    //get the value of the old matrix before replacing the fragment
    void setupInititalPosition() {

        setImageMatrix(oldMatrix);
    }
    ...
    //rombeye business logic
    public void setIsExpanded(boolean isExpanded) {
        this.isExpanded = isExpanded;
    }

    public void setIsMultiple(boolean isMultiple, RectF rectF) {
        this.isMultiple = isMultiple;
        this.firstSelectedRectCrop.set(rectF.left, rectF.top,
    ...
    public void setIsOldFragment(boolean isOldFragment, Matrix matrix) {
        this.isOldFragment = isOldFragment;
