 ...
 */
package com.alphaapps.instapickercropper.internal.loader;
...
import com.alphaapps.instapickercropper.MimeType;
...
public class AlbumMediaLoader extends CursorLoader {
    ...
    private static final String SELECTION_ALL =
            "(" + MediaStore.Files.FileColumns.MEDIA_TYPE + "=?"
                    ...
                    + " AND (" + MediaStore.Video.VideoColumns.DURATION + " < 31000"
                    + " OR " + MediaStore.Files.FileColumns.MEDIA_TYPE + " = " + MediaStore.Files.FileColumns.MEDIA_TYPE_IMAGE + ")";
    ...
    public static CursorLoader newInstance(Context context, Album album, boolean capture) {
        ...
        if (album.isAll()) {
            if (SelectionSpec.getInstance().onlyShowImages()) {
                ...
                selectionArgs = new String[SelectionSpec.getInstance().mimeTypeSet.size()];
                String selectionIn = MediaStore.Files.FileColumns.MIME_TYPE + " in (";
                List<String> selections = new ArrayList<>();
                for (MimeType mime : SelectionSpec.getInstance().mimeTypeSet) {
                    selectionIn += "?, ";
                    selections.add(mime.getmMimeTypeName());
                }
                selectionIn = selectionIn.substring(0, selectionIn.length() - 2) + ")";


                selectionIn += " AND " + MediaStore.MediaColumns.SIZE + ">0"
                        + " AND (" + MediaStore.Video.VideoColumns.DURATION + " < 31000"
                        + " OR " + MediaStore.Files.FileColumns.MEDIA_TYPE + " = " + MediaStore.Files.FileColumns.MEDIA_TYPE_IMAGE + ")";

                selection = selectionIn;
                selectionArgs = selections.toArray(selectionArgs);
