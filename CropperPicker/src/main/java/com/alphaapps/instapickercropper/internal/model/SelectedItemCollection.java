...
import com.alphaapps.instapickercropper.internal.utils.PhotoMetadataUtils;
...
//    public static final String STATE_SELECTED = "state_selected";
    ...
    static CropperPickerFragment listener;
//    public HashMap<String, CroppingViewHolder> croppingItems;
    ...
    public static SelectedItemCollection getInstance(CropperPickerFragment listener) {
        if (instance == null) {
            instance = new SelectedItemCollection();
        }
     SelectedItemCollection.listener = listener;
        return instance;
    }
...
//            croppingItems=new HashMap<>();
        } else {
            List<Item> saved = bundle.getParcelableArrayList(STATE_SELECTION);
//            croppingItems = (HashMap<String, CroppingViewHolder>) bundle.getSerializable(STATE_SELECTED);
...
//        outState.putSerializable(STATE_SELECTED,croppingItems);
...
//        bundle.putSerializable(STATE_SELECTED, croppingItems);
...
//            EventBus.getDefault().post(new MessageEvent("id", item.getContentUri().toString()));
        }
        return added;
    }
    ...
    public boolean remove(Item item) {
        ...
        if (removed) {
            ...
            if (mItems.size() == 0) {
                {
//                    EventBus.getDefault().post(new MessageEvent("id", item.getContentUri().toString()));
                    SelectedItemCollection.getInstance().setLastSelectedItem(null);

                }
            } else {
                Item[] items = new Item[mItems.size()];
                mItems.toArray(items);
                SelectedItemCollection.getInstance().setLastSelectedItem(items[items.length - 1]);
                EventBus.getDefault().post(new MessageEvent("id", items[items.length - 1].getContentUri().toString(), true));
            }
        }
        return removed;
    }
    ...
    public List<String> asListOfString(Context mContext) {
    ...
    public IncapableCause isAcceptable(Item item, Context mContext) {
        if (!SelectionSpec.getInstance().countable) {
            Iterator<Item> iterator = mItems.iterator();
            while (iterator.hasNext()) {
                Item i = iterator.next();
                if (!item.equals(i) && isSelected(i)) {
                    Log.v("media", "new item added");
                    remove(i);
                }
            }
        } else {
...
//            }
    ...
    public SelectedItemCollection setLastSelectedItem(Item lastSelectedItem) {
...
//    public HashMap<String, CroppingViewHolder> getCroppingItems() {
//        return croppingItems;
//    }

//    public SelectedItemCollection setCroppingItems(HashMap<String, CroppingViewHolder> croppingItems) {
//        this.croppingItems = croppingItems;
//        return this;
//    }
    ...
    public Item getSingleItem(String itemURI){
        for(Item item: getItems()){
            if (item.getContentUri().toString().equals(itemURI))
                return item;
        }
        return null;
    }
