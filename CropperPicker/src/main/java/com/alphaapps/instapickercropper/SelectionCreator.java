...
import com.alphaapps.instapickercropper.engine.ImageEngine;
import com.alphaapps.instapickercropper.filter.Filter;
import com.alphaapps.instapickercropper.internal.entity.CaptureStrategy;
...
import com.alphaapps.instapickercropper.ui.CropperPickerFragment;
...
public final class SelectionCreator {
    ...
    //////////////
    private final InstaCropperPicker mInstaCropperPicker;
     ...
     *
     * @param instaCropperPicker   a requester context wrapper.
     * @param mimeTypes MIME type set to select.
     */
    SelectionCreator(InstaCropperPicker instaCropperPicker, @NonNull Set<MimeType> mimeTypes, boolean mediaTypeExclusive) {
        mInstaCropperPicker = instaCropperPicker;
     ...
     * 1. com.alphaapps.CropperPicker.R.style.CropperPicker_Zhihu;
     * 2. com.alphaapps.CropperPicker.R.style.CropperPicker_Dracula
     * you can define a custom theme derived from the above ones or other themes.
     *
     * @param themeId theme resource id. Default value is com.alphaapps.CropperPicker.R.style.CropperPicker_Zhihu.
     * @return {@link SelectionCreator} for fluent API.
     */
     ...
     * 1. {@link com.alphaapps.instapickercropper.engine.impl.GlideEngine}
     * 2. {@link com.alphaapps.instapickercropper.engine.impl.PicassoEngine}
     ...
     */
    public void forResult(int requestCode, boolean showCamera, String userName, String userAvatar, boolean isSingleImage, ArrayList<Uri> recentImageUri, boolean showGalleryIcon, String fragmentsStates) {
        Activity activity = mInstaCropperPicker.getActivity();
        if (activity == null) {
            return;
        }
        Intent intent = new Intent(activity, CropperPickerActivity.class);
        intent.putExtra(CropperPickerActivity.SHOW_CAMERA, showCamera);
        intent.putExtra(CropperPickerActivity.USER_NAME_KEY, userName);
        intent.putExtra(CropperPickerActivity.USER_AVATAR_KEY, userAvatar);
        intent.putExtra(CropperPickerActivity.IS_SINGLE_IMAGE_KEY, isSingleImage);
        intent.putExtra(CropperPickerActivity.RECENT_IMAGE_PATH_KEY, recentImageUri);
        intent.putExtra(CropperPickerActivity.SHOW_Camera_ICON, showGalleryIcon);
        intent.putExtra(CropperPickerFragment.FRAGMENTS_STATUS, fragmentsStates);

        Fragment fragment = mInstaCropperPicker.getFragment();
        if (fragment != null) {
    ...
    public void forResult(int requestCode, boolean showCamera,  boolean isSingleImage, ArrayList<Uri> recentImageUri, boolean showGalleryIcon) {
        Activity activity = mInstaCropperPicker.getActivity();
        if (activity == null) {
            return;
        }
        ...
        intent.putExtra(IS_SINGLE_IMAGE_KEY, isSingleImage);
        intent.putExtra(RECENT_IMAGE_PATH_KEY, recentImageUri);
        intent.putExtra(SHOW_Camera_ICON, showGalleryIcon);
